...
* 인덱서
  ...
  * `this`와 구분 기호 `[` 및 `]` 사이에 작성된 매개 변수 목록을 합쳐서 구성원 이름으로 사용한다는 점을 제외하고 속성처럼 선언
  * 매개 변수는 인덱서의 접근자에서 사용할 수 있음
  * 읽기/쓰기, 읽기 전용 및 쓰기 전용일 수 있으며 인덱서의 접근자는 가상일 수 있음
  * 인덱서는 오버로드 될 수 있음
  * 하나의 클래스는 매개 변수의 수와 형식이 다른 여러 인덱서를 선언할 수 있음

* 이벤트
  * 이벤트는 클래스 또는 개체가 알림을 제공할 수 있도록 하는 멤버
  * 이벤트는 선언에 `event`키워드가 포함됨
  * 형식이 대리자 형식이어야 한다는 점을 제외하고 필드처럼 선언
  * 이벤트는 대리자 형식의 필드처럼 동작(이벤트가 추상이 아니고 접근자를 선언하지 않을 경우)
  * 이벤트에 추가된 이벤트 처리기를 나타내는 대리자에 대한 참조를 저장
  * 이벤트 처리기가 없는 경우 필드는 `null`
  * 이벤트 처리기는 `+=` 연산자를 사용하여 추가되고, `-=`연산자를 사용하여 제거됨
  * 이벤트의 기본 스토리지를 제어하려고 하는 고급 시나리오의 경우 이벤트 선언에서 속성의 `set` 접근자와 비슷한 `add`, `remove` 접근자를 명시적으로 제공할 수 있음

* 연산자
  * 연산자는 클래스 인스턴스에 특정 식 연사자를 적용하는 것의 의미를 정의하는 멤버
  * 정의할 수 있는 세 가지 종류의 연산자
    * 단항 연산자
    * 이항 연산자
    * 변환 연산자
  * 모든 연산자는 `public` 및 `static`으로 선언해야 함
  * `MyList<T>`의 경우 `operator ==` 및 `operator !=`를 선언
  * 재정의된 연산자는 해당 연산자를 인스턴스에 적용하는 식에 새로운 의미를 부여
...
* 표현식
  * 문서 참고할 것
    * [식](https://learn.microsoft.com/ko-kr/dotnet/csharp/language-reference/language-specification/expressions#1184-logical-negation-operator)

* 문
  * 문서 참고할 것
    * [문](https://learn.microsoft.com/ko-kr/dotnet/csharp/tour-of-csharp/program-building-blocks#statements)